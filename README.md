...
`u` and `a` are the hyperparameters in the TPBN prior for the local scale parameters. `u=0.5` and `a=0.5` correspond to the popular horseshoe prior of Carvalho et al. (2010).  Meanwhile, `tau` is the global hyperparameter in the TPBN prior. The default for `tau` is 1/(p * sqrt(n * log(n))). If this quantity becomes lower than `1e-5`, then `tau` is set as `1e-5` for numerical stability reasons. If p is must larger than n and all of the signals in the data are very weak, then the method may select a null model. In this case, it is recommended that the user increase the value of `tau`.
